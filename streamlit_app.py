...
def main():
    ...
    with col1:
        unit_system = st.radio("Units:", ["English", "Metric"], index=0)  # Default English for lbs
    with col2:
    ...
    with col3:
        if unit_system == "Metric":
            ...
            height_in = st.number_input("Height (in):", min_value=48.0, max_value=96.0, value=68.0, step=0.5)
            height_m = inches_to_m(height_in)
    with col4:
        current_almi = st.number_input("Current ALMI (kg/m²):", min_value=3.0, max_value=15.0, value=7.0, step=0.1)
    with col5:
        experience_level = st.selectbox("Training Experience:", ["Beginner", "Intermediate", "Advanced"])
    ...
    if current_almi:
        ...
        fig_percentile = create_percentile_visualization(current_almi, short_term_target, gender, "ALMI")
